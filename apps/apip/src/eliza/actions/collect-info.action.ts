 ...
 */
const STOP_WORDS =
  /\b(?:j['’]ai|je\s+suis|j['’]habite|il\s+|elle\s+|on\s+|nous\s+|vous\s+|c['’]est|oui|non|accident|douleur|fracture|saigne|malaise|chute)\b/i;
const ADDRESS_REGEX =
...
const buildFullAddress = (
  ...
  const hasCity = ville
    ? new RegExp(`\\b${ville.replace(/\s+/g, "\\s+")}\\b`, "i").test(normalized)
    : false;
  ...
  handler: async (userMessage: string, context: any): Promise<ActionResult> => {
    ...
    if (adresse) {
      ...
      const confirmPattern =
        /\b(oui|exact|c['’]est\s*(?:bien|ça|ca)|correct|d['’]accord|ok)\b/i;
