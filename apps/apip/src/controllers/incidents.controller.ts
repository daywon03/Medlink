...
export class IncidentsController {
  ...
  private mapIncident(row: any) {
    ...
    const statusRaw = String(row.status ?? row.state ?? row.etat ?? 'clos').toLowerCase();
    const status: IncidentStatus =
      statusRaw.includes('clos') || statusRaw.includes('closed') || statusRaw.includes('termin')
        ? 'clos'
    ...
    const location = this.parseMaybeJson(
      row.patient_location ?? row.location ?? row.position ?? triage?.patient_location
    );
    const lat =
      row.lat ??
      row.latitude ??
    ...
    const symptoms = this.parseSymptoms(row.symptoms ?? row.symptom_list ?? row.symptomes);
    ...
    return {
      id,
      createdAt,
      createdAtRaw: row.date_heure ?? row.created_at ?? row.createdAt ?? row.date ?? undefined,
      updatedAtRaw: row.updated_at ?? row.updatedAt ?? row.closed_at ?? undefined,
      status,
      priority,
      title,
      locationLabel,
  ...
  private parsePriority(value: any): 1 | 2 | 3 | 4 | 5 {
    ...
    if (typeof value === 'number' && value >= 1 && value <= 5) return value as 1 | 2 | 3 | 4 | 5;
    const str = String(value).toUpperCase();
    const map: Record<string, 1 | 2 | 3 | 4 | 5> = { P0: 1, P1: 2, P2: 3, P3: 4, P4: 5, P5: 5 };
    return map[str] ?? 3;
  }
